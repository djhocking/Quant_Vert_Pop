...
The package `tidyr` can also be used to reshape and organize data. Install and load the `tidyr` package. Look up the help menu for the functions `spread()` and `gather()` and try them to go back and forth using the data `d` as we did with reshape.
...
Exercise 7: figure out how to use a call to sweep() to do the same thing as scale(x,center=TRUE,scale=FALSE).
